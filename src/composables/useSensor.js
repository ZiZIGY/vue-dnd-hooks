import { checkCollision, getBoundingBox, getCenter, getDistance, } from '../utils/geometry';
import { isDescendant } from '../utils/dom';
import { useDnDStore } from './useDnDStore';
import { usePointer } from './usePointer';
export const useSensor = (elementRef) => {
    const store = useDnDStore();
    const { onPointerStart, onPointerMove, onPointerEnd } = usePointer(elementRef);
    let animationFrameId = null;
    const getDraggingElements = (draggableElement) => {
        // Проверяем, содержится ли draggableElement в selectedElements
        const isDraggableInSelection = store.selectedElements.value.some((element) => element.node === draggableElement);
        ...
        const element = store.elements.value.find((element) => element.node === draggableElement);
        if (!element)
            return [];
        return [
            {
    ...
    const getOverlapPercent = (boxA, boxB) => {
        const xOverlap = Math.max(0, Math.min(boxA.x + boxA.width, boxB.x + boxB.width) -
            Math.max(boxA.x, boxB.x));
        const yOverlap = Math.max(0, Math.min(boxA.y + boxA.height, boxB.y + boxB.height) -
    ...
    const detectCollisions = () => {
        ...
        const isPointerInActiveContainer = containerRect &&
        ...
        const collidingElements = store.elements.value
            .filter((element) => {
            if (!element.node ||
                activeDragNodes.some((dragNode) => dragNode &&
                    isDescendant(element.node, dragNode))) {
                return false;
            }
            if (element.groups.length) {
                const isCompatible = !store.draggingElements.value.some((draggingElement) => {
                    if (!draggingElement.groups.length)
                        return false;
                    return !draggingElement.groups.some((group) => element.groups.includes(group));
                });
                if (!isCompatible)
                    return false;
            }
            const rect = getBoundingBox(element.node);
            return rect && containerRect && checkCollision(rect, containerRect);
        })
            .map((element) => {
            const rect = getBoundingBox(element.node);
            const elementCenter = getCenter(rect);
            const isPointerInElement = pointerX >= rect.x &&
            ...
            const depth = store.elements.value.filter((parent) => parent !== element &&
                parent.node &&
                element.node &&
                isDescendant(element.node, parent.node)).length;
            return {
                element,
                isPointerInElement,
                overlapPercent,
        ...
        const collidingZones = store.zones.value
            ...
            if (!zone.node ||
                activeDragNodes.some((dragNode) => dragNode &&
                    isDescendant(zone.node, dragNode))) {
                return false;
            }
            ...
            const rect = getBoundingBox(zone.node);
            return rect && containerRect && checkCollision(rect, containerRect);
        })
            .map((zone) => {
            const rect = getBoundingBox(zone.node);
            ...
            const depth = store.zones.value.filter((parent) => parent !== zone &&
                parent.node &&
                zone.node &&
                isDescendant(zone.node, parent.node)).length;
            return {
                zone,
                isPointerInElement,
                overlapPercent,
    ...
    const deactivate = () => {
        ...
        if (store.hovered.zone.value) {
            ...
            store.draggingElements.value.forEach((element) => element.events.onEnd?.(store));
        }
