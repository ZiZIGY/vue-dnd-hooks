...
export const useZoneManager = (options) => {
    ...
    const isAllowed = computed(() => {
        ...
        if (!isDragging.value)
            return false;
        const currentZone = zones.value.find((zone) => zone.node === elementRef.value);
        if (!currentZone?.groups.length)
            return true;
        return !draggingElements.value.some((element) => {
            if (!element.groups.length)
                return false;
            return !element.groups.some((group) => currentZone.groups.includes(group));
        });
    });
    const registerZone = () => {
        if (!elementRef.value)
    ...
    const unregisterZone = () => {
        if (!elementRef.value)
            throw new Error('elementRef is not set');
        const index = zones.value.findIndex((zone) => zone.node === elementRef.value);
        if (index !== -1)
