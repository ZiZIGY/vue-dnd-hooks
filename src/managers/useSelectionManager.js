 ...
 */
export const useSelectionManager = (elementRef) => {
    const { selectedElements, elements } = useDnDStore();
    /** Current element from the DnD store */
    const element = computed(() => elements.value.find((element) => element.node === elementRef.value));
    /** Whether the current element is selected */
    const isSelected = computed(() => selectedElements.value.some((element) => element.node === elementRef.value));
    /** Whether the current element contains any selected elements */
    const isParentOfSelected = computed(() => {
        if (!elementRef.value)
            return false;
        return selectedElements.value.some((selected) => selected.node &&
            isDescendant(selected.node, elementRef.value));
    });
    ...
    const hasSelectedParent = computed(() => {
        if (!elementRef.value)
            return false;
        return selectedElements.value.some((selected) => selected.node &&
            isDescendant(elementRef.value, selected.node));
    });
    /** Removes the current element from selection */
    const handleUnselect = () => {
        if (!element.value)
            return;
        selectedElements.value = selectedElements.value.filter((element) => element.node !== elementRef.value);
    };
    /**
    ...
    const handleSelect = () => {
        ...
        if (isParentOfSelected.value) {
            selectedElements.value = selectedElements.value.filter((selected) => selected.node &&
                !isDescendant(selected.node, elementRef.value));
        }
