<script setup lang="ts">
  ...
  const transitionRef = ref<HTMLDivElement | null>();
...
<template>
  <div
    ref="transitionRef"
    class="fixed inset-0 overflow-hidden border-dashed border-2 border-emerald-600"
  >
    <div
      class="text-emerald-600 text-center absolute top-0 size-20 w-full z-50"
    >
      <svg
      ...
      >
        <g fill="currentColor">
          <g opacity=".2">
            <path
              fill-rule="evenodd"
              d="M4.456 13.267a47 47 0 0 1-.98-1.559c-1.13-1.866.24-4.205 2.398-4.208V5.375A1.875 1.875 0 0 1 8.63 3.72a1.875 1.875 0 0 1 3.645-.448a1.875 1.875 0 0 1 2.849 1.603v.21q.235-.084.5-.085h.375c1.035 0 1.875.84 1.875 1.875v4.56l.004.065c0 2.136-.806 3.774-2.043 4.874C14.608 17.465 12.985 18 11.378 18c-3.263 0-5.393-2.187-6.922-4.733M7.749 4.5c.483 0 .875.392.875.875V7.5a.5.5 0 0 0 1 0V3.875a.875.875 0 1 1 1.75 0V7a.5.5 0 0 0 1 0V4.875a.875.875 0 0 1 1.75 0V7.5a.5.5 0 0 0 1 0v-1a.5.5 0 0 1 .5-.5h.375c.483 0 .875.392.875.875V11.5q0 .033.004.064c-.016 1.831-.706 3.172-1.707 4.062c-1.022.909-2.4 1.374-3.793 1.374c-2.73 0-4.6-1.805-6.07-4.254l-.016-.026a46 46 0 0 1-.96-1.53c-.715-1.18.14-2.687 1.542-2.69V10a.5.5 0 0 0 1 0V5.375c0-.483.392-.875.875-.875"
              clip-rule="evenodd"
            />
            <path
    ...
    <div
      :style="{
        transform: `translate(-${lastScrollPosition.x}px, -${lastScrollPosition.y}px)`,
      }"
      class="z-40"
    >
      <component :is="previousRouteComponent" />
    </div>
  </div>
</template>
